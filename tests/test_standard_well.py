"""Quyu tənlikləri — OPM tipli quyu modeli, MƏRHƏLƏ 2.
...
from imex2d.simulation.implicit.three_phase_residual import ThreePhaseFluidState
...
def _setup(nx=5, ny=5, pressure=213.5, sw=0.35):
    ...
    state = ThreePhaseState(p, np.full(n, sw), np.zeros(n), np.zeros(n, bool))
    fluid = ThreePhaseFluidState(
        mu_w=pvt.water_viscosity(p), mu_o=pvt.oil_viscosity(p),
        mu_g=pvt.gas_viscosity(p), bw=pvt.water_fvf(p), bo=pvt.oil_fvf(p),
        bg=pvt.gas_fvf(p), rs=state.solution_gor(pvt),
        krw=relperm.krw(state.water_saturation),
        kro=relperm.kro_three_phase(state.water_saturation,
                                    state.gas_saturation),
        krg=relperm.krg(state.gas_saturation))
...
def test_water_injector_produces_no_oil_or_gas():
    """Su vurucusu yalnız su verir — vurulan fazanın mobilliyi."""
    model, connections, state, fluid, well_model, wells = _setup()
    rates = well_model.perforation_rates(state, fluid, wells)
    assert abs(rates.per_well_oil["INJ-1"]) < 1e-12
    assert abs(rates.per_well_gas["INJ-1"]) < 1e-12


def test_producer_gas_includes_both_free_and_dissolved():
    """Qaz debiti = sərbəst + Rs·neft (bax mərhələ 6a/6b).

    DİQQƏT: doymuş vəziyyət üçün flüid xassələri DƏ həmin vəziyyətdən
    qurulmalıdır — əks halda `krg=0`, `Rs=0` qalır və test mənasız
    olur (ilk yazılışın səhvi).
    """
    model, connections, state, fluid, well_model, wells = _setup()
    n = state.ncell
    saturated = ThreePhaseState(state.pressure, state.water_saturation,
    ...
    saturated_fluid = ThreePhaseFluidState(
        mu_w=pvt.water_viscosity(p), mu_o=pvt.oil_viscosity(p),
        mu_g=pvt.gas_viscosity(p), bw=pvt.water_fvf(p), bo=pvt.oil_fvf(p),
        bg=pvt.gas_fvf(p), rs=saturated.solution_gor(pvt),
        krw=relperm.krw(saturated.water_saturation),
        kro=relperm.kro_three_phase(saturated.water_saturation,
                                    saturated.gas_saturation),
        krg=relperm.krg(saturated.gas_saturation))

    rates = well_model.perforation_rates(saturated, saturated_fluid, wells)
    dissolved = saturated_fluid.rs[0] * rates.per_well_oil["PROD-1"]
    free = rates.per_well_gas["PROD-1"] - dissolved
    assert abs(free) > 0, "sərbəst qaz həddi sıfırdır"
    assert abs(dissolved) > 0, "həll olmuş qaz həddi sıfırdır"
...
def _jacobian_setup(saturated: bool):
    """Doymuş VƏ doymamış rejimlər ayrıca yoxlanılır.

    DƏRS (A7_PLAN.md, v60): əvvəllər Jakobian yalnız TAM DOYMUŞ
    vəziyyətdə doğrulanırdı və doymamış rejimdəki səhv uzun müddət
    gizli qaldı. İndi hər iki rejim məcburidir.
    """
    ...
    if saturated:
        state = ThreePhaseState(base.pressure, base.water_saturation,
                                np.full(n, 0.12), np.ones(n, bool))
    else:
        state = ThreePhaseState(base.pressure, base.water_saturation,
    ...
    def build_fluid(s):
        ...
        return ThreePhaseFluidState(
            mu_w=pvt.water_viscosity(p), mu_o=pvt.oil_viscosity(p),
            mu_g=pvt.gas_viscosity(p), bw=pvt.water_fvf(p),
            bo=pvt.oil_fvf(p), bg=pvt.gas_fvf(p), rs=s.solution_gor(pvt),
...
def _check_reservoir_column(saturated: bool, variable: int, tolerance=1e-6):
    (model, connections, well_model, wells, state, build_fluid,
     jacobian) = _jacobian_setup(saturated)
    cell = next(c for c in connections if not c.is_injector).cell
    blocks = jacobian.blocks(state, build_fluid(state), wells)

    step = 1e-5
    vector = state.to_vector()
    forward = vector.copy(); forward[cell * 3 + variable] += step
    backward = vector.copy(); backward[cell * 3 + variable] -= step
    ...
    numeric = np.array([
        ...
        (rates_f.gas[cell] - rates_b.gas[cell]) / (2 * step)])
...
def test_rate_pressure_derivative_undersaturated():
    _check_reservoir_column(saturated=False, variable=0)


def test_rate_pressure_derivative_saturated():
    _check_reservoir_column(saturated=True, variable=0)


def test_rate_water_saturation_derivative_undersaturated():
    _check_reservoir_column(saturated=False, variable=1)


def test_rate_water_saturation_derivative_saturated():
    _check_reservoir_column(saturated=True, variable=1)


def test_rate_third_variable_derivative_undersaturated():
    _check_reservoir_column(saturated=False, variable=2)


def test_rate_third_variable_derivative_saturated():
    _check_reservoir_column(saturated=True, variable=2)


def _check_bhp_column(saturated: bool):
    (model, connections, well_model, wells, state, build_fluid,
     jacobian) = _jacobian_setup(saturated)
    producer = next(c for c in connections if not c.is_injector)
    cell = producer.cell
...
def test_rate_bhp_derivative_undersaturated():
    """`∂q/∂p_bhp` — köhnə modeldə YOX idi (BHP orada sabit idi).
    Məhz bu bağlantı quyunu sistemin bir hissəsinə çevirir."""
    _check_bhp_column(saturated=False)


def test_rate_bhp_derivative_saturated():
    _check_bhp_column(saturated=True)


def test_bhp_control_derivative_is_one():
    """BHP idarəsində `R = p_bhp − hədəf` → ∂R/∂p_bhp = 1."""
    (model, connections, well_model, wells, state, build_fluid,
     jacobian) = _jacobian_setup(saturated=False)
    blocks = jacobian.blocks(state, build_fluid(state), wells)
...
def _coupled_setup(nx=4, ny=4):
    ...
    solver = CoupledNewtonSolver(model, relperm, pvt, NewtonLinearSolver(),
                                 grid, connections)
    n = model.ncell
    reservoir = ThreePhaseState(np.linspace(200.0, 225.0, n),
                                np.full(n, 0.35), np.zeros(n),
                                np.zeros(n, bool))
...
def test_coupled_system_size_includes_wells():
    ...
    assert state.size == state.reservoir.ncell * 3 + state.wells.count
...
def test_coupled_jacobian_matches_finite_difference():
    ...
    reference = solver.flux.upstream_masks(previous.reservoir, previous_fluid)
    dt = 1.0

    def residual_of(candidate):
        value, _, _ = solver.compute_residual(candidate, previous,
                                              previous_fluid, dt, reference)
        return value

    _, fluid, _ = solver.compute_residual(state, previous, previous_fluid,
                                          dt, reference)
    jacobian = solver.assemble_jacobian(state, fluid, dt, reference)
    ...
    for column in range(vector.size):
        ...
        numeric = (residual_of(CoupledState.from_vector(
                       forward, state.reservoir.is_saturated, names))
                  - residual_of(CoupledState.from_vector(
                       backward, state.reservoir.is_saturated, names))
                  ) / (2 * step)
...
def test_coupled_solver_never_raises():
    """Mövcud həllediciylə eyni zəmanət (bax `ThreePhaseNewtonSolver`)."""
    solver, state = _coupled_setup()
